...
console.log('🔍 Test Query #3: Verify all active employees have mandatory balances\n')
...
for (const emp of employees) {
  ...
  // Get mandatory types for this org
  ...
  if (!mandatoryTypes || mandatoryTypes.length === 0) {
    console.log(`⚠️  Org "${emp.organizations.name}" has no mandatory types`)
    continue
  }
...
if (percentComplete === '100.0') {
  console.log('✅ Test Query #3 Complete - 0% missing balances')
} else {
