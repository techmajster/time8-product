...
interface DeleteLeaveRequestSheetProps {
  isOpen: boolean
  onClose: () => void
...
export function DeleteLeaveRequestSheet({
  isOpen,
  onClose,
  onDelete,
  leaveRequest
}: DeleteLeaveRequestSheetProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!reason.trim()) {
      showError('Uzasadnienie jest wymagane')
      return
    }
        ...
        <div className="flex flex-col h-full">
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 items-end p-6 h-full">
            ...
            <SheetTitle className="sr-only">
              Usuń wniosek o urlop
            </SheetTitle>
            <SheetDescription className="sr-only">
              {tSheet('deleteLeaveRequest')}
            </SheetDescription>
            ...
            <div className="flex justify-between gap-2 w-full pt-4">
              ...
              >
                {isLoading ? 'Usuwanie...' : 'Usuń wniosek'}
              </Button>
            </div>
          </form>
        </div>
